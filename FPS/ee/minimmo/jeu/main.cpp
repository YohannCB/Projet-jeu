       ...
       // Calcul le temps delta d'une image.
        ...
        if(!movedWithJoystick)
        {
            ...
            core::line3df ray = smgr->getSceneCollisionManager()->getRayFromScreenCoordinates(
                receiver.GetMouseState().Position, MaCamera->Camera);
...
n().Y * PI/180.0f ) );                                                                               
      ...
      if (receiver.IsKeyDown(KEY_KEY_M))
	{
	  ...
	  MaCamera->Camera->setPosition(vector3df(node->getPosition().X - 20, node->getPosition().Y, node->getPosition().Z - 20)); //EE
	}
