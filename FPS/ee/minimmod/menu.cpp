...
{
  //menu->addItem(L"Jouer", -1, true, true);
  //menu->addItem(L"Touches", -1, true, true);
  //menu->addItem(L"Quitter", -1, true, false);
...
int main()
{
	...
	IrrlichtDevice * device = createDevice(driverType, core::dimension2d<u32>(6040, 4800));	// >= irrlicht 1.6
	...
	//gui::IGUIContextMenu* menuBar = env->addMenu (0, -0);
	// some button below menu to see if mouse-clicks are correctly eaten by the menu    (x,y,longueur,largeur)
	core::rect<s32> rectBtn(400, 150, 999, 200);
	context.CreateRightClickMenues = env->addButton (rectBtn, 0, E_RIGHT_CLICK_MENUES, L"JOUER");
	context.CreateRightClickMenues->setIsPushButton(true);

	rectBtn = core::rect<s32>(400, 250, 999, 300);
	context.EatCloseEvent = env->addButton (rectBtn, 0, E_RIGHT_CLICK_MENUES, L"TOUCHES");
	context.EatCloseEvent->setIsPushButton(true);

	rectBtn = core::rect<s32>(400, 450, 999, 500);
	context.EatCloseEvent = env->addButton (rectBtn, 0, E_RIGHT_CLICK_MENUES, L"QUITTER");
	context.EatCloseEvent->setIsPushButton(true);
